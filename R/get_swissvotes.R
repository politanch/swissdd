...
get_swissvotes <- function(DB=T, savecitation=F, codebook=F){
  ...
  if(DB){
    swissvotesDB <- read.csv("https://swissvotes.ch/page/dataset/swissvotes_dataset.csv",
                             sep=";", stringsAsFactors = F)
  }


  if(codebook) browseURL("https://swissvotes.ch/page/dataset/codebook-de.pdf")


  if(DB) {

    message("To cite swissvotes data in publications, please use:\n
            Linder, Wolf, Christian Bolliger und Yvan Rielle (2010): Verzeichnisse der Literatur, der Quellen und der Abkuerzungen für die Kurzbeschreibungen zu den Abstimmungen 1848–2007. In: Linder, Wolf, Christian Bolliger und Yvan Rielle (Hg.): Handbuch der eidgenoessischen Volksabstimmungen 1848–2007. Bern: Haupt. S. 713–729.\n

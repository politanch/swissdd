...
First, we extract data from the database and select the variables we need to create our prediction. For some reason we may believe that the National Council represents the people. Therefore, we should see a similar pattern in yes shares in the National Council and later on in the actual yes share of a popular vote. 

First, we download and prettify the data. After doint that we calculate the share of yes votes in the National Council and create a scatterplot to inspect if our intuition about the relationship can be seen in the data.

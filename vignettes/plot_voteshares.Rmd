...
The function `get_geodata` allows users to download geographic information for different administrative units in Switzerland. The function returns a *spatial feature* `data.frame`. A user should specify the level of administrative units using the argument  `geolevel`. The following options are available:
